...
#[frb(ignore)]
#[derive(Debug, Default)]
pub struct ThorchainMetadata {
    pub inbounds: Vec<ThorchainInbound>,
    pub pools: Vec<ThorchainPool>,
}

fn parse_thorchain_asset_id(asset: &str) -> Option<(&str, &str, &str)> {
    let (chain, rest) = asset.split_once('.')?;
    if chain.is_empty() || rest.is_empty() {
        return None;
    }
    match rest.split_once('-') {
        Some((symbol, addr)) => Some((chain, symbol, addr)),
        None => Some((chain, rest, "")),
    }
}
...
#[frb(ignore)]
pub async fn fetch_thorchain_metadata() -> Result<ThorchainMetadata, String> {
    let client = zilpay::reqwest::Client::new();
    let (inbounds_res, pools_res) = zilpay::tokio::join!(
        fetch_inbound_addresses_raw(&client),
        fetch_pools_raw(&client),
    );
    Ok(ThorchainMetadata {
        inbounds: inbounds_res?,
        pools: pools_res?,
    })
}
...
impl ThorchainSwapQuote {
    pub fn into_quote_result(self) -> super::ExchangeQuoteResult {
        super::ExchangeQuoteResult {
            expected_amount_out: self.expected_amount_out,
            min_amount_in: self.recommended_min_amount_in,
            fees: super::ExchangeFees {
                total: self.fees.total,
                outbound: self.fees.outbound,
                liquidity: self.fees.liquidity,
                affiliate: self.fees.affiliate,
                slippage_bps: self.fees.slippage_bps,
                total_bps: self.fees.total_bps,
            },
            timing: super::ExchangeTiming {
                inbound_seconds: self.inbound_confirmation_seconds,
                outbound_seconds: self.outbound_delay_seconds,
                total_seconds: self.total_swap_seconds,
            },
            warning: self.warning,
            notes: self.notes,
            tx_params: super::ExchangeTxParams::Thorchain(super::ThorchainTxParams {
                inbound_address: self.inbound_address,
                router: self.router,
                memo: self.memo,
                expiry: self.expiry,
                recommended_gas_rate: self.recommended_gas_rate,
...
async fn fetch_pools_raw(
    client: &zilpay::reqwest::Client,
) -> Result<Vec<ThorchainPool>, String> {
    let mut last_err = String::new();
    for base_url in THORCHAIN_BASE_URLS {
        match try_fetch_pools(base_url, client).await {
            Ok(result) => return Ok(result),
            Err(e) => last_err = e,
        }
    }
...
async fn try_fetch_pools(
    base_url: &str,
    client: &zilpay::reqwest::Client,
) -> Result<Vec<ThorchainPool>, String> {
    let url = format!("{base_url}/thorchain/pools");
    let resp = client.get(&url).send().await.map_err(|e| format!("{e}"))?;
    if !resp.status().is_success() {
        return Err(resp.text().await.unwrap_or_default());
    }
    let raw: Vec<PoolRaw> = resp.json().await.map_err(|e| format!("{e}"))?;
    let mut result = Vec::with_capacity(raw.len());
    for item in raw {
        let asset = item.asset;
        let (chain, symbol, token_addr) = match parse_thorchain_asset_id(&asset) {
            Some(parsed) => parsed,
            None => continue,
        };
        ...
        let available = PoolStatus::from(item.status.as_str()) == PoolStatus::Available;
        let decimals = item.decimals.min(u8::MAX as i64) as u8;
        result.push(ThorchainPool {
