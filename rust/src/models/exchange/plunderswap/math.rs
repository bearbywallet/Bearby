...
pub(super) fn resolve_pair(
    ...
    let wzil_hex = wzil.to_string();
    let (resolved_out, is_native_out) = resolve_out(to_asset, chain_id)?;
    let token_out = if is_native_out {
        Cow::Borrowed(wzil_hex.as_str())
    } else {
        resolved_out
    };
    let token_in = resolve_in(is_native_in, &wzil_hex, from_asset);
    let is_wrap_unwrap =
        token_in.eq_ignore_ascii_case(&wzil_hex) && token_out.eq_ignore_ascii_case(&wzil_hex);
...
mod tests {
    ...
    #[test]
    fn resolve_pair_substitutes_wzil_for_native_edges() -> Result<(), String> {
        let wzil = addr(0x94);
        let token = addr(0x42).to_string();
        let (token_in, token_out, native_out, wrap) =
            resolve_pair(32_769, wzil, NATIVE_SENTINEL, &token, true)?;
        assert_eq!(token_in, wzil.to_string());
        assert_eq!(token_out, token);
        assert!(!native_out);
        assert!(!wrap);

        let (token_in, token_out, native_out, wrap) =
            resolve_pair(32_769, wzil, &wzil.to_string(), NATIVE_SENTINEL, false)?;
        assert_eq!(token_in, wzil.to_string());
        assert_eq!(token_out, wzil.to_string());
        assert!(native_out);
        assert!(wrap);
        Ok(())
    }
    ...
    fn resolve_pair_rejects_cross_chain_output() {
        let err = resolve_pair(32_769, addr(0x94), NATIVE_SENTINEL, "1:0xabc", true)
            .err()
            .unwrap_or_default();
