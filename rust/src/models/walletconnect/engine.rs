...
use super::rpc::{
    ...
    SessionDeleteParams, SessionEventParams, SessionProposeParams, SessionProposeResult,
    SessionRequestParams, SessionSettleParams, SessionUpdateParams, SettledNamespace,
...
impl WcEngine {
    ...
    pub async fn approve(
        ...
        // 1. subscribe session topic first
        let _sub_id = self.relay.subscribe(session_topic.clone()).await?;

        // 2. respond on pairing topic
        ...
        // 3. settle on session topic
        let expiry = now_secs().saturating_add(SEVEN_DAYS);
        let settle = SessionSettleParams {
            relay: Relay::irn(),
            namespaces: namespaces.clone(),
            controller: Participant {
                public_key: self_pub_hex.clone(),
                metadata: self.self_meta.clone(),
            },
            expiry,
        };
        self.publish_request(&session_topic, "wc_sessionSettle", &settle, &SESSION_SETTLE)
            .await?;
        ...
        // 4. activate pairing
        {
            ...
            state.sessions.insert(
                ...
                Session {
                    ...
                    self_pub: self_pub_hex,
                    peer_pub: pending.proposal.proposer_pub,
                    peer_meta: pending.proposal.proposer_meta,
                    namespaces,
                    expiry,
                    acknowledged: false,
                },
