...
import 'package:bearby/modals/watch_asset_modal.dart';
...
class Web3EIP1193Handler {
  ...
  Future<void> _handleEthSendTransaction(
    ...
    try {
      ...
      final typedTxParams = Map<String, Object?>.from(txParams);
      final valueAmount = evmValueAmount(typedTxParams);
      final to = typedTxParams[kParamTo]?.toString();
      final transactionRequest = buildEvmTransactionRequest(
        txParams: typedTxParams,
        nativeToken: mbToken,
        chainHash: appState.chain?.chainHash ?? BigInt.zero,
        signerAddr: appState.account?.addr,
        title: title ?? kEvmTransactionTitle,
        icon: message.icon,
      );
